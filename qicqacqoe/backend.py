...
class Qtic:
    ...
    def teleportation(self, i, j, m):
        # Reinitialize Bell pair qubits: H on m alone, then CNOT, gives the
        # EPR pair (|00> + |11>)/sqrt(2). An H on both would give |++>,
        # which CNOT leaves unchanged - no entanglement, no teleportation.
    ...
    def simulate(self, shot=1024, seed=None):
        # Transpile specifically for the backend
        transpiled_circuit = transpile(self.circuit, self.backend)

        # Run job (a fixed seed makes the whole game reproducible)
        job = self.backend.run(transpiled_circuit, shots=shot, seed_simulator=seed)
